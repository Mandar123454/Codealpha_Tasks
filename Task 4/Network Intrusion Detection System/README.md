...
### Snort on Windows (quick start)
1. Install Npcap (WinPcap API-compatible mode enabled).
2. Install Snort for Windows (see official Snort docs or maintained builds).
3. Open an elevated PowerShell and verify interfaces:
  ```powershell
  snort -W
  ```
4. Run Snort with the provided config and your interface index:
  ```powershell
  # From repo root
  cd "Task 4/Network Intrusion Detection System"
  snort -i <interface_index> -c snort_config/snort.conf -A console
  ```
  Replace `<interface_index>` with the number from `snort -W`.
